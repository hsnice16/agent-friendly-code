...
- **Security**: parameterised SQL, `dangerouslySetInnerHTML` only for the existing server-built JSON-LD scripts (with the `<` → `\u003c` escape preserved), `rel="noopener noreferrer"` on external links, clone safety.
